...
% State constraint He*e <= he
He = [1 0; -1 0; 0 1; 0 -1]; % 4 inequalities for x1 and x2 (rectangle)
he = [5; 5; 5; 5]; % Constraints: |x1| <= 5, |x2| <= 5

% Define constraint set for control v s.t. Hv*v <= hv
Hv = [1 0; -1 0; 0 1; 0 -1]; % 4 inequalities for v1 and v2 (rectangle)
hv = [5; 5;5;5]; % Constraints: |v1| <= 2, |v2| <= 2

% Add to state constraints
He = [He; -Hv*K]; % K is the feedback gain
he = [he; hv];

% Bounded set for disturbance w: Hw*w <= hw
Hw = [1 0; -1 0; 0 1; 0 -1]; % 4 inequalities for w1 and w2 (rectangle)
hw = [1; 1; 1; 1]; % Constraints: |w1| <= 1, |w2| <= 1
...
while isconverged == false
    ...
    h_rpi = h_rpi_new;
    % New values
    [H_pre, h_pre] = compute_robust_pre(Acl, H_rpi, h_rpi, Hw, hw, options); % 有扰动
...
function [H_new, h_new] = compute_intersection(H1, h1, H2, h2, options)
    ...
    while i <= size(H_new,1) && size(H_new,1)>1
        ...
        % Add Hi*x \leq hi+1 for bounded solution，是为了在求解时，确保考虑到当前的 Hi 约束，同时还留出一定的余地（即 +1）。
        A = [H_new; Hi];
        b_loc = [h_new;hi+1];        
...
function [H_pre, h_pre] = compute_robust_pre(A,He,he,Hw,hw, options)
    ...
    H_pre = He*A;
    for i=1:size(he,1)
        [~,h_pre(i)] = linprog(He(i,:)',Hw,hw,[],[],[],[],options); % 计算结果为当前约束在考虑扰动时的最大偏移量
    end
    h_pre = he+h_pre';
end
